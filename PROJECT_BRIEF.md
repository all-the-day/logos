...
- 三态：查看（全文/填空、中英对照、章节快速跳转 chips）→ 背诵输入 → 结果比对 + 四键评分
...
1. **结果页塞研读内容**：`/learn` 结果态接入注解/笔记 —— 明确不做，另开 plan
2. **Undo 评分重构**：`LearnClient.handleUndo` 用"评分前快照 + PUT 整卡恢复"，脆弱，需重构（已加日期边界校验，架构重构待规划）
3. ~~**`getDueCards` 新卡泄漏**~~ —— **已修复**（2026-08-10，补 `state != "new"` 过滤，含失败测试）
4. ~~**SQLite DateTime 混合存储**~~ —— **已确认一致**（当前 Prisma 写入路径均存整数毫秒；dev.db 审计 433 张卡全 INTEGER，无需迁移）
5. **`lib/auth.ts:7` ESLint warning**：`userDb` 未使用（留作单独 chore）
6. **`date-fns` 依赖未使用**（可清理，另开 chore）
7. **`compare.ts` 空原文缺陷**：`it.fails` 已标注，修复后移除 `.fails`
...
- 单元测试：`lib/fsrs`（14）、`lib/compare`（17，含全组合还原 + `it.fails` 记录空原文缺陷）、`lib/date`（10）
...
- 当前共 **56 个用例**；`npm run test` / `test:coverage`（约 85% stmts）/ `test:db:reset`
