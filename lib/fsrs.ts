...
/** 预测某评级的间隔天数（不修改卡片） */
export function previewInterval(card: FsrsCard, rating: Rating): number {
  if (![1, 2, 3, 4].includes(rating)) {
    throw new Error(`Invalid rating: ${rating}`);
  }
  const stability = calcStability(card, rating);
  return calcInterval(card, rating, stability);
}

/** 计算可检索概率 R(t) */
export function getRetention(card: FsrsCard, now: Date = new Date()): number {
  if (!card.lastReview || card.state === STATE.NEW) return 1.0;
  const elapsedDays =
    (now.getTime() - card.lastReview.getTime()) / DAY_MS;
  if (elapsedDays <= 0 || card.stability <= 0) return 1.0;
  return Math.exp(-elapsedDays / (card.stability * INTERVAL_FACTOR));
}

/** 准确率 → FSRS 评级 */
export function accuracyToRating(accuracy: number): Rating {
  if (accuracy < 0.5) return RATING.AGAIN;
  if (accuracy < 0.8) return RATING.HARD;
  if (accuracy < 0.95) return RATING.GOOD;
  return RATING.EASY;
}
